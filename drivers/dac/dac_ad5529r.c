				 ...
				 const struct dac_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->resolution != AD5529R_RESOLUTION) {
		LOG_ERR("invalid resolution %u (expected %u)",
			channel_cfg->resolution, AD5529R_RESOLUTION);
		return -EINVAL;
	}
	...
	ret = ad5529r_write_reg(dev, AD5529R_REG_OUT_RANGE_CH(channel_cfg->channel_id),
				config->output_range);
	if (ret != 0) {
		LOG_ERR("failed to set output range for channel %u: %d",
	...
	LOG_DBG("channel %u setup complete (range=0x%02X)",
		channel_cfg->channel_id, config->output_range);
...
static int ad5529r_init(const struct device *dev)
{
	...
	ret = ad5529r_read_reg(dev, AD5529R_REG_PRODUCT_ID_L, &product_id);
	if (ret != 0) {
		LOG_ERR("failed to read product ID: %d", ret);
		return ret;
	}

	LOG_INF("AD5529R initialized (product_id=0x%04X, device_id=%u)",
...
#define AD5529R_INST_DEFINE(inst)                                                       \
	...
	DEVICE_DT_INST_DEFINE(inst, ad5529r_init, NULL, &data_##inst, &config_##inst,   \
			      POST_KERNEL, CONFIG_DAC_AD5529R_INIT_PRIORITY,            \
			      &ad5529r_driver_api);
